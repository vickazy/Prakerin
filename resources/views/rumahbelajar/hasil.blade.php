...
<div class="navbar-collapse collapse pull-right">
	...
	<form action="{{ url('/hasil') }}" class="navbar-form pull-right" role="search">
        ...
        <div class="row">
        <div class="col-md-8">
        @foreach ($nama as $berita)
		<article>
		<div class="col-md-5">
			<img style="width:300px; height:200px;"src="{{ asset($berita->image) }}" class="img-responsive">
		</div>
					<h3>{!! $berita->judul !!}</h3> 
					<span class="glyphicon glyphicon-time"></span> {{ $berita->created_at }}
			        
			          <hr>
					
			          <p class="lead">{{$berita->read_more.' ...'}}</p>
					  <p class="text-right">{!!link_to_route('berita.tampil','Read More',$berita->id)!!}</p>
